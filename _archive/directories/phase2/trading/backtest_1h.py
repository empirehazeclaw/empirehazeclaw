...
print("🚀 Starte 1-Jahres Backtest (1h Chart, Pagination)...")
...
# 1 Jahr zurück (um Rate Limits und RAM im Rahmen zu halten, aber viele Trades zu generieren)
...
for symbol in coins:
    ...
    # Pagination loop für mehr als 1000 Kerzen
    while True:
        try:
            ...
            # Verhindere Endlos-Schleife falls API immer dieselbe letzte Kerze zurückgibt
            if all_ohlcv and ohlcv[-1][0] == all_ohlcv[-1][0]:
                break
                
            all_ohlcv.extend(ohlcv)
            current_since = ohlcv[-1][0] + 1  # timestamp + 1ms
            
            # Wenn wir im aktuellen Datum angekommen sind, breche ab
            if len(ohlcv) < 1000:
                break
        except Exception as e:
            print(f"   Fehler beim Fetching: {e}")
            break
    ...
    # Behalte nur eindeutige Zeitstempel
...
for current_date in btc_dates:
    ...
    if len(positions) < 3: # Max 3 Trades
        for symbol in coins:
            ...
            # Strategie: RSI unter 35 (auf dem 1h Chart kommt das viel öfter vor)
...
print("\n=== 📊 1-JAHRES BACKTEST (1H CHART) ERGEBNISSE ===")
