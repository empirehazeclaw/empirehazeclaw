...
def calculate_loop_score() -> float:
    ...
    # Base score from validation rate (or neutral 0.5 if no validations yet)
    if val_total > 0:
        val_rate = val_success / val_total
    else:
        val_rate = 0.5  # Neutral until we have data
    
    cross_hits = state.get("cross_pattern_hits", 0)
    cross_miss = state.get("cross_pattern_misses", 0)
    cross_total = cross_hits + cross_miss
    cross_rate = cross_hits / max(cross_total, 1) if cross_total > 0 else 0.5
    
    feedback_proc = state.get("feedback_processed", 0)
    
    # Iteration bonus (loop is running)
    iteration = state.get("iteration", 0)
    iteration_factor = min(iteration / 20, 1.0) * 0.2  # Max 0.2 from iterations
    
    # Score components
    score = (
        val_rate * 0.35 +           # 35% - validation success
        cross_rate * 0.25 +                   # 25% - cross-pattern hits
        min(feedback_proc / 30, 1.0) * 0.2 + # 20% - feedback processed
        iteration_factor +                   # 20% - iteration running
        0.1  # Base 10% floor
    )
